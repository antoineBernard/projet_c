...
void deplacementReineFouTour()
{
    ...
    int k = caseArrivee.ligne, l = caseArrivee.colonne;
...
void testSiEchecEtMat()
{
    ...
    */
    int i,j,l;
    int echecEtMatRoiBlanc; //a mettre en variable globale
    int roiNoireSauve = 0;
    int roiBlancSauve = 0;
    ...
    for(i = 0 ; i < 8 ; i++)
    {
        ...
        {   
            ...
            if(t[i][j] == 'c') && i == 1 && t[i+1][j] == ' ' &&(t[i+2][j] == ' ' || //déplacement de 2 coup du Pion
                                             t[i+2][j] == ' '))
            {
                /* je deplace la piece pour tester si echec*/
                t[i+2][j] = t[i][j];
