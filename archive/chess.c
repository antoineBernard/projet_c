...
void deplacementpiece()
{
  ...
  while(ECHEC == 1)
  {
      ...
      charColonneDepart = toupper(charColonneDepart);

      verifCaseChoisi(charColonneDepart,
                    p_ligneDepart, 
                    p_intColonneDepart);
       ligneDepart--;                    
       pieceTrouve = t[ligneDepart][intColonneDepart] ;

       //rangeeDepart = ligneDepart ;
           
                // A PARTIR D'ICI ON A ACCES AUX VARIABLES intColonneDepart, pieceTrouve, LigneDepart pour le reste des fonctions 
                   ...
                   int *p_intColonne)
{
    ...
    {
      char bidon;
      pieceTrouve = t[*p_ligne-1][*p_intColonne];
